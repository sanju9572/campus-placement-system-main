...
function Spinner({ color = "#10b981", size = 28 }) {
  return (
    <motion.div animate={{ rotate: 360 }} transition={{ repeat: Infinity, duration: 1, ease: "linear" }}
      style={{ width: size, height: size, borderRadius: "50%", border: `3px solid ${color}25`, borderTopColor: color, margin: "0 auto" }} />
  );
}
...
function Toast({ message, type = "success", onClose }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: -20, x: "-50%" }} animate={{ opacity: 1, y: 0, x: "-50%" }} exit={{ opacity: 0, y: -20, x: "-50%" }}
      ...
      <Icon d={type === "success" ? IC.check : IC.warning} size={16} color="#fff" sw={2.5} />
      {message}
    </motion.div>
  );
}
...
function Btn({ children, onClick, color = "indigo", loading = false, style = {}, ...props }) {
  const bgs = { indigo: "linear-gradient(135deg,#4f46e5,#10b981)", green: "linear-gradient(135deg,#10b981,#059669)", red: "linear-gradient(135deg,#ef4444,#dc2626)", amber: "linear-gradient(135deg,#f59e0b,#d97706)", gray: "linear-gradient(135deg,#6b7280,#4b5563)" };
  return (
...
function StatCard({ title, value, icon, color, darkMode, delay = 0 }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} transition={{ delay, duration: 0.4 }}
      whileHover={{ y: -4, boxShadow: `0 12px 28px ${c.ac}30` }}
    ...
    <header style={{ background: D.headerBg(darkMode), boxShadow: "0 4px 24px rgba(79,70,229,0.28)", position: "sticky", top: 0, zIndex: 100 }}>
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "10px 32px", borderBottom: "1px solid rgba(255,255,255,0.12)" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                ...
                <motion.div initial={{ opacity: 0, y: -8, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -8, scale: 0.95 }}
                  ...
                  {notifications.map(n => (
                    <div key={n.id} style={{ padding: "10px 16px", borderBottom: `1px solid ${D.border(darkMode)}`, background: n.read ? "transparent" : darkMode ? "rgba(16,185,129,0.08)" : "#f0fdf4", display: "flex", alignItems: "center", gap: 10 }}>
                      <div style={{ width: 30, height: 30, borderRadius: 8, background: darkMode ? "rgba(16,185,129,0.15)" : "#d1fae5", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                        <Icon d={IC[n.icon] || IC.bell} size={14} color="#10b981" />
                      </div>
          ...
          {/* Profile */}
          <div style={{ position: "relative" }}>
            <motion.img whileHover={{ scale: 1.06 }} src={admin.avatar} alt="admin" onClick={() => setShowMenu(!showMenu)}
              style={{ width: 40, height: 40, borderRadius: "50%", border: "2px solid rgba(255,255,255,0.5)", cursor: "pointer", objectFit: "cover" }} />
              ...
              {showMenu && (
                <motion.div initial={{ opacity: 0, y: -8, scale: 0.95 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -8, scale: 0.95 }}
                  ...
                  <button onClick={() => { fileInputRef.current?.click(); setShowMenu(false); }}
                    style={{ display: "flex", alignItems: "center", gap: 8, width: "100%", padding: "10px 14px", background: "none", border: "none", cursor: "pointer", fontSize: 12, color: "#6366f1", fontWeight: 600 }}>
                    <Icon d={IC.camera} size={14} color="#6366f1" /> Change Photo
                  </button>
                  <button onClick={onLogout}
                    style={{ display: "flex", alignItems: "center", gap: 8, width: "100%", padding: "10px 14px", background: "none", border: "none", cursor: "pointer", fontSize: 12, color: "#ef4444", fontWeight: 600 }}>
        ...
        {NAV_ITEMS.map(({ label, id, icon }) => {
          ...
          return (
            <motion.button key={id} onClick={() => onNav(id)} whileHover={{ background: "rgba(255,255,255,0.12)" }}
              ...
              <Icon d={icon} size={13} color={active ? "#fff" : "rgba(255,255,255,0.65)"} />
              {label}
            </motion.button>
          );
        })}
...
export default function AdminDashboard() {
  ...
  const admin = {
    name:   localStorage.getItem("name")  || "Admin Coordinator",
    email:  localStorage.getItem("email") || "admin@indusuni.ac.in",
    avatar: adminPic,
  };
  ...
  const [notifications, setNotifications] = useState([
    { id: 1, text: "Welcome to the Admin Panel!", read: false, icon: "dashboard" },
  ]);
  const markAllRead = () => setNotifications(p => p.map(n => ({ ...n, read: true })));
  const addNotif = (text, icon = "bell") => setNotifications(p => [{ id: Date.now(), text, read: false, icon }, ...p]);

  // ── API: FETCH ALL OPPORTUNITIES ───────────────────────────────────────────
  // GET /opportunities?skip=0&limit=50
  ...
  const fetchOpportunities = useCallback(async () => {
    ...
    try {
      const res = await API.get("/opportunities?skip=0&limit=50");
      setOpportunities(res.data);
    } catch (err) {
      setOppsError(err.response?.data?.detail || "Failed to load opportunities");
    } finally { setOppsLoading(false); }
  }, []);

  useEffect(() => { fetchOpportunities(); }, [fetchOpportunities]);

  // ── API: POST NEW OPPORTUNITY ──────────────────────────────────────────────
  // POST /companies/{company_id}/opportunities
  const EMPTY_OPP = { company_id: "", title: "", description: "", location: "", ctc_lpa: "", application_deadline: "", additional_criteria: "", jdMode: "text" };
  ...
  const handleSaveOpportunity = async () => {
    if (!oppForm.company_id.trim()) { showToast("Company ID is required", "error"); return; }
    if (!oppForm.title.trim())      { showToast("Job title is required", "error"); return; }
    setSavingOpp(true);
    try {
      const payload = {
        title:                oppForm.title,
        description:          oppForm.description         || undefined,
        location:             oppForm.location            || undefined,
        ctc_lpa:              oppForm.ctc_lpa ? parseFloat(oppForm.ctc_lpa) : undefined,
        application_deadline: oppForm.application_deadline
          ? new Date(oppForm.application_deadline).toISOString()
          : new Date(Date.now() + 30 * 86400000).toISOString(),
        additional_criteria:  oppForm.additional_criteria || undefined,
      };

      if (editingOppId) {
        // PATCH /opportunities/{opportunity_id}
        await API.patch(`/opportunities/${editingOppId}`, payload);
        showToast(`"${oppForm.title}" updated successfully!`, "success");
        addNotif(`Opportunity updated: ${oppForm.title}`, "edit");
        setEditingOppId(null);
      } else {
        // POST /companies/{company_id}/opportunities
        await API.post(`/companies/${oppForm.company_id.trim()}/opportunities`, payload);
        showToast(`"${oppForm.title}" posted successfully!`, "success");
        addNotif(`New opportunity: ${oppForm.title}`, "company");
      }

      setOppForm(EMPTY_OPP);
      await fetchOpportunities();
      scrollTo("sec-opportunities");
    } catch (err) {
  ...
  // ── API: PATCH OPPORTUNITY STATUS ──────────────────────────────────────────
  // PATCH /opportunities/{opportunity_id}  body: { status: "active" }
  const handlePublish = async (opp) => {
    ...
    try {
      ...
      showToast(`"${opp.title}" is now ${newStatus}`, "success");
      addNotif(`${opp.title} set to ${newStatus}`, "shield");
    } catch (err) {
  ...
  const handleEditOpportunity = (opp) => {
    setOppForm({
      ...
      ctc_lpa:              opp.ctc_lpa?.toString() || "",
      application_deadline: opp.application_deadline ? new Date(opp.application_deadline).toISOString().slice(0, 16) : "",
  ...
  // ── API: APPLICATIONS FOR AN OPPORTUNITY ───────────────────────────────────
  ...
  const handleViewOpportunity = async (opp) => {
    setViewOpp(opp);
    setViewAppsLoading(true);
    try {
      const res = await API.get(`/opportunities/${opp.id}/applications`);
      setViewApps(res.data);
    } catch (err) {
      showToast(err.response?.data?.detail || "Failed to load applications", "error");
    } finally { setViewAppsLoading(false); }
  };

  // ── API: UPDATE APPLICATION STATUS ─────────────────────────────────────────
  // PATCH /applications/{application_id}/status  body: { status: "shortlisted" }
  const handleUpdateAppStatus = async (appId, newStatus) => {
    ...
    try {
      ...
      if (newStatus === "offered" || newStatus === "accepted") {
        addNotif(`${res.data.student_name || "A student"} received an offer from ${viewOpp?.title}!`, "trophy");
      }
    } catch (err) {
      showToast(err.response?.data?.detail || "Update failed", "error");
  ...
  const [placedForm, setPlacedForm] = useState({ name: "", email: "", company: "", position: "", package: "", batch: "2025", greeting: "", photo: null });
  const placedPhotoRef = useRef(null);

  const handleAddPlacedStudent = () => {
    if (!placedForm.name || !placedForm.company || !placedForm.position) { showToast("Name, company and position are required", "error"); return; }
    const s = { id: Date.now(), ...placedForm, photo: placedForm.photo || `https://i.pravatar.cc/100?u=${placedForm.name}`, greeting: placedForm.greeting || `Congratulations ${placedForm.name}! Placed at ${placedForm.company}.` };
    setPlacedStudents(p => [s, ...p]);
    setPlacedForm({ name: "", email: "", company: "", position: "", package: "", batch: "2025", greeting: "", photo: null });
    showToast(`${s.name} added to Wall of Fame!`, "success");
    addNotif(`${s.name} added to placed students!`, "trophy");
  };

  // stats
  const totalApps  = opportunities.reduce((a, o) => a + (o.application_count || 0), 0);
  ...
  return (
    <div style={{ minHeight: "100vh", background: D.pageBg(darkMode), fontFamily: "'Outfit','Segoe UI',sans-serif", color: D.textPri(darkMode), transition: "background 0.35s, color 0.35s" }}>
      ...
      <TopBar darkMode={darkMode} setDarkMode={setDarkMode} notifications={notifications} markAllRead={markAllRead}
        activeSection={activeSection} onNav={scrollTo} admin={admin}
        onPhotoChange={handleAdminPhotoChange} fileInputRef={fileInputRef} onLogout={handleLogout} />
      ...
      <main style={{ maxWidth: 1200, margin: "0 auto", padding: "28px 20px 60px" }}>
        ...
        <div id="sec-dashboard">
          <motion.div initial={{ opacity: 0, y: -18 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div style={{ display: "flex", gap: 12, position: "relative" }}>
              <motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.97 }} onClick={() => scrollTo("sec-addopp")}
                style={{ background: "rgba(255,255,255,0.18)", border: "1px solid rgba(255,255,255,0.3)", color: "#fff", borderRadius: 12, padding: "9px 18px", fontWeight: 700, cursor: "pointer", fontSize: 13, display: "flex", alignItems: "center", gap: 6 }}>
                <Icon d={IC.plus} size={14} color="#fff" /> Post Opportunity
              </motion.button>
              <motion.button whileHover={{ scale: 1.04 }} whileTap={{ scale: 0.97 }} onClick={() => scrollTo("sec-placed")}
                style={{ background: "rgba(255,255,255,0.12)", border: "1px solid rgba(255,255,255,0.3)", color: "#fff", borderRadius: 12, padding: "9px 18px", fontWeight: 700, cursor: "pointer", fontSize: 13, display: "flex", alignItems: "center", gap: 6 }}>
        ...
        <div id="sec-addopp" style={{ scrollMarginTop: 90 }}>
          ...
          <Card darkMode={darkMode}>
            {/* API hint */}
            <div style={{ marginBottom: 16, padding: "10px 14px", background: darkMode ? "rgba(99,102,241,0.08)" : "#eef2ff", borderRadius: 10, border: `1px solid ${D.border(darkMode)}`, fontSize: 12, color: D.textSec(darkMode) }}>
              {editingOppId
                ? <><strong style={{ color: D.textPri(darkMode) }}>PATCH</strong> <code style={{ background: darkMode ? "rgba(255,255,255,0.1)" : "#e0e7ff", padding: "1px 6px", borderRadius: 4 }}>/opportunities/{"{opportunity_id}"}</code></>
                : <><strong style={{ color: D.textPri(darkMode) }}>POST</strong> <code style={{ background: darkMode ? "rgba(255,255,255,0.1)" : "#e0e7ff", padding: "1px 6px", borderRadius: 4 }}>/companies/{"{company_id}"}/opportunities</code></>
              }
            </div>

            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(240px,1fr))", gap: 14 }}>
              <InputField label="Company ID (UUID) *" darkMode={darkMode} name="company_id" placeholder="e.g. 123e4567-e89b-12d3-..." value={oppForm.company_id} onChange={handleOppFormChange} disabled={!!editingOppId} style={editingOppId ? { opacity: 0.5, cursor: "not-allowed" } : {}} />
              ...
              <InputField label="CTC (LPA) — number" darkMode={darkMode} name="ctc_lpa" type="number" step="0.5" placeholder="e.g. 12.5" value={oppForm.ctc_lpa} onChange={handleOppFormChange} />
              ...
              <InputField label="Additional Criteria (optional)" darkMode={darkMode} name="additional_criteria" placeholder="e.g. Must have ML project experience" value={oppForm.additional_criteria} onChange={handleOppFormChange} />
            </div>
            ...
            <div style={{ marginTop: 14 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
                ...
                <div style={{ display: "flex", background: darkMode ? "rgba(16,185,129,0.1)" : "#d1fae5", borderRadius: 8, padding: 3, gap: 3 }}>
                  {["text", "pdf"].map(mode => (
                    <button key={mode} onClick={() => setOppForm(p => ({ ...p, jdMode: mode }))}
                      style={{ padding: "4px 10px", borderRadius: 6, border: "none", cursor: "pointer", fontSize: 11, fontWeight: 600, background: oppForm.jdMode === mode ? "#10b981" : "transparent", color: oppForm.jdMode === mode ? "#fff" : D.textSec(darkMode), transition: "all 0.2s" }}>
              ...
              {oppForm.jdMode === "text"
                ...
                : (
                  <div>
                    ...
                    <motion.button whileTap={{ scale: 0.96 }} onClick={() => jdPdfRef.current?.click()}
                      style={{ width: "100%", padding: "20px", borderRadius: 12, border: `2px dashed ${D.border(darkMode)}`, background: D.inputBg(darkMode), cursor: "pointer", display: "flex", flexDirection: "column", alignItems: "center", gap: 8 }}>
                      <Icon d={IC.pdf} size={28} color="#10b981" />
                      <div style={{ fontSize: 13, fontWeight: 600, color: D.textPri(darkMode) }}>
                        {oppForm.description?.startsWith("[PDF]") ? `✓ ${oppForm.description}` : "Click to upload JD PDF"}
                      </div>
            ...
            <div style={{ marginTop: 16, display: "flex", gap: 10, flexWrap: "wrap" }}>
              <Btn onClick={handleSaveOpportunity} loading={savingOpp}>
                <Icon d={editingOppId ? IC.check : IC.plus} size={14} color="#fff" />
                {editingOppId ? "Update Opportunity" : "Post Opportunity"}
              </Btn>
              {editingOppId && (
                <Btn color="amber" onClick={() => { setOppForm(EMPTY_OPP); setEditingOppId(null); }}>Cancel Edit</Btn>
              )}
        ...
        <div id="sec-opportunities" style={{ scrollMarginTop: 90 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <motion.button whileTap={{ scale: 0.95 }} onClick={fetchOpportunities}
          ...
          {oppsLoading && <div style={{ textAlign: "center", padding: 40 }}><Spinner /><div style={{ marginTop: 12, color: D.textSec(darkMode), fontSize: 13 }}>Loading from GET /opportunities...</div></div>}
          ...
          {!oppsLoading && !oppsError && (
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(280px,1fr))", gap: 16 }}>
                  ...
                  <div style={{ padding: "16px 18px" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 10 }}>
                      <div>
                        ...
                        <div style={{ fontSize: 11, color: D.textSec(darkMode), marginTop: 2, fontFamily: "monospace" }}>Company: {opp.company_id?.slice(0, 12)}...</div>
                      </div>
                      <span style={{
                        background: opp.status === "active" ? "#d1fae5" : opp.status === "closed" ? "#fee2e2" : "#fef3c7",
                        color: opp.status === "active" ? "#065f46" : opp.status === "closed" ? "#7f1d1d" : "#92400e",
                        padding: "3px 9px", borderRadius: 20, fontSize: 10, fontWeight: 700, textTransform: "capitalize"
                      }}>{opp.status}</span>
                    </div>
                    ...
                    <div style={{ display: "flex", gap: 8 }}>
                      {/* Publish/Unpublish toggle — PATCH /opportunities/{id} */}
                      <motion.button whileTap={{ scale: 0.96 }} onClick={() => handlePublish(opp)}
                        style={{ flex: 1, background: opp.status === "active" ? "linear-gradient(135deg,#f59e0b,#d97706)" : "linear-gradient(135deg,#10b981,#059669)", color: "#fff", border: "none", borderRadius: 10, padding: "8px", fontWeight: 700, cursor: "pointer", fontSize: 12, display: "flex", alignItems: "center", justifyContent: "center", gap: 5 }}>
                        <Icon d={opp.status === "active" ? IC.eye : IC.check} size={13} color="#fff" />
                        {opp.status === "active" ? "Unpublish" : "Publish"}
                      </motion.button>
                      {/* View applications */}
                      <motion.button whileTap={{ scale: 0.95 }} onClick={() => handleViewOpportunity(opp)}
                        style={{ background: darkMode ? "rgba(16,185,129,0.15)" : "#d1fae5", border: "none", borderRadius: 10, padding: "8px 12px", cursor: "pointer", display: "flex", alignItems: "center", gap: 5, color: darkMode ? "#6ee7b7" : "#065f46", fontSize: 12, fontWeight: 600 }}>
                        <Icon d={IC.apps} size={13} color={darkMode ? "#6ee7b7" : "#065f46"} /> Applicants
                      </motion.button>
                      {/* Edit */}
                      <motion.button whileTap={{ scale: 0.95 }} onClick={() => handleEditOpportunity(opp)}
                        style={{ background: darkMode ? "rgba(99,102,241,0.15)" : "#eef2ff", border: "none", borderRadius: 10, padding: "8px 10px", cursor: "pointer" }}>
                        <Icon d={IC.edit} size={14} color="#6366f1" />
                      </motion.button>
                      {/* Delete */}
                      <motion.button whileTap={{ scale: 0.95 }} onClick={() => handleDeleteOpportunity(opp.id, opp.title)}
                        style={{ background: "rgba(239,68,68,0.1)", border: "none", borderRadius: 10, padding: "8px 10px", cursor: "pointer" }}>
              ...
              {opportunities.length === 0 && (
                <div style={{ gridColumn: "1/-1", textAlign: "center", padding: 48, color: D.textMuted(darkMode) }}>
                  No opportunities yet. Post one above!
                </div>
              )}
            </div>
          )}
        ...
        <div id="sec-placed" style={{ scrollMarginTop: 90 }}>
          ...
          <Card darkMode={darkMode} style={{ marginBottom: 20 }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))", gap: 12 }}>
              {[["Student Name *","name","Full name"],["Email","email","student@indusuni.ac.in"],["Company *","company","e.g. TCS"],["Position *","position","e.g. Software Developer"],["Package","package","e.g. 8 LPA"],["Batch","batch","e.g. 2025"]].map(([label, name, placeholder]) => (
                <InputField key={name} label={label} darkMode={darkMode} name={name} placeholder={placeholder} value={placedForm[name]} onChange={e => setPlacedForm(p => ({ ...p, [e.target.name]: e.target.value }))} />
              ))}
            </div>
            <div style={{ marginTop: 12 }}>
              <TextareaField label="Greeting Message" darkMode={darkMode} name="greeting" placeholder="e.g. Congratulations Rahul! Your dedication led you to TCS." value={placedForm.greeting} onChange={e => setPlacedForm(p => ({ ...p, greeting: e.target.value }))} style={{ minHeight: 60 }} />
            </div>
            <div style={{ marginTop: 12, display: "flex", alignItems: "center", gap: 12 }}>
              ...
              <motion.button whileTap={{ scale: 0.96 }} onClick={() => placedPhotoRef.current?.click()}
                style={{ padding: "9px 16px", borderRadius: 10, border: `1.5px dashed ${D.border(darkMode)}`, background: D.inputBg(darkMode), color: D.textSec(darkMode), cursor: "pointer", fontSize: 12, display: "flex", alignItems: "center", gap: 6 }}>
                <Icon d={IC.camera} size={14} color={D.textSec(darkMode)} />
                {placedForm.photo ? "Photo Uploaded ✓" : "Upload Photo"}
              </motion.button>
            ...
            <div style={{ marginTop: 14, display: "flex", gap: 10 }}>
              ...
              <Btn color="amber" onClick={() => setPlacedForm({ name: "", email: "", company: "", position: "", package: "", batch: "2025", greeting: "", photo: null })}>Clear</Btn>
            </div>
          </Card>
            ...
            {placedStudents.map(s => (
              <motion.div key={s.id} whileHover={{ y: -4 }}
                ...
                <div style={{ padding: "10px 14px 16px", textAlign: "center" }}>
                  ...
                  <div style={{ fontSize: 12, color: D.textSec(darkMode) }}>{s.company}</div>
                  <div style={{ margin: "8px 0", padding: "6px 10px", background: darkMode ? "rgba(16,185,129,0.1)" : "#f0fdf4", borderRadius: 8, fontSize: 11, color: D.textSec(darkMode), fontStyle: "italic" }}>🎉 {s.greeting}</div>
                  <div style={{ display: "flex", gap: 5, justifyContent: "center", marginBottom: 10 }}>
                    <span style={{ background: "#d1fae5", color: "#065f46", padding: "2px 9px", borderRadius: 20, fontSize: 11, fontWeight: 600 }}>{s.package}</span>
                    <span style={{ background: "#eef2ff", color: "#4338ca", padding: "2px 9px", borderRadius: 20, fontSize: 11, fontWeight: 600 }}>{s.batch}</span>
                  </div>
                  <motion.button whileTap={{ scale: 0.95 }} onClick={() => setPlacedStudents(p => p.filter(x => x.id !== s.id))}
                    style={{ background: "rgba(239,68,68,0.1)", border: "none", borderRadius: 8, padding: "5px 12px", color: "#ef4444", cursor: "pointer", fontSize: 11, fontWeight: 600, display: "flex", alignItems: "center", gap: 4, margin: "0 auto" }}>
                    <Icon d={IC.trash} size={11} color="#ef4444" /> Remove
                  </motion.button>
                </div>
              </motion.div>
            ))}
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 18 }}>
                <div>
                  ...
                  <div style={{ fontSize: 12, color: "#10b981", marginTop: 2 }}>
                    GET /opportunities/{viewOpp.id}/applications
                  </div>
                </div>
                <motion.button whileTap={{ scale: 0.9 }} onClick={() => setViewOpp(null)}
                  ...
                  : viewApps.map(app => {
                      ...
                      return (
                        <div key={app.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "12px 14px", borderRadius: 12, marginBottom: 8, background: darkMode ? "rgba(16,185,129,0.05)" : "#f9faff", border: `1px solid ${D.border(darkMode)}`, flexWrap: "wrap", gap: 10 }}>
                          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                            <div style={{ width: 38, height: 38, borderRadius: 10, background: darkMode ? "rgba(16,185,129,0.15)" : "#d1fae5", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 800, fontSize: 14, color: "#10b981" }}>
                              {app.student_name?.[0] || "?"}
                            </div>
                            <div>
                              <div style={{ fontWeight: 600, fontSize: 14, color: D.textPri(darkMode) }}>{app.student_name}</div>
                              <div style={{ fontSize: 11, color: D.textSec(darkMode) }}>
                                {app.student_email}
                                {app.student_cgpa && <span> · CGPA: {app.student_cgpa}</span>}
                          ...
                          {/* Status dropdown — PATCH /applications/{id}/status */}
                          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                            <select
                              value={app.status}
                              disabled={updatingAppId === app.id}
                              onChange={e => handleUpdateAppStatus(app.id, e.target.value)}
                              style={{ padding: "6px 10px", borderRadius: 8, border: `1px solid ${D.border(darkMode)}`, background: D.inputBg(darkMode), color: D.textPri(darkMode), fontSize: 12, cursor: "pointer", fontWeight: 600 }}>
                              {APP_STATUS_OPTIONS.map(s => (
                                <option key={s} value={s}>{APP_STATUS_CONFIG[s]?.label || s}</option>
                              ))}
                            </select>
